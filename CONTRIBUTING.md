...
兩個理由：這些視角不可能互相矛盾，而且滑一下鼠標不該讓編譯器再跑一次。
判決更新時 `lensChanged.fire()` 叫 VS Code 重新拿 lens。

**符號另外存一份（`symbolsOf`），而且只在有符號的時候才覆寫。**
打字的人一分鐘會把檔案弄壞好幾次，每次都清空的大綱等於沒有，
所以那裡保留「最後一份好的」。判決不能這樣做——過期的判決是對螢幕上那個檔案說謊。

跳定義的位置也全部來自編譯器的 `symbols`（1.15.0）。理由跟 hover 的說明一樣：
要知道 `mercy` 是什麼，得解析 `define mercy := kill`，而那是別名軸。
插件唯一做的位置工作是**在編譯器指定的那一行上，用單字邊界找出那個已知名字的欄位**——
拿到一個字串去找它自己，安全；判斷那個字串是什麼意思，不安全。
...
| | 回答什麼 | 例子 |
| --- | --- | --- |
| **環境式**（打字就出現） | **會不會** | 讀得懂嗎？精靈擋不擋？是不是 exploit？ |
| **動作式**（你按了才發生） | **怎麼會** | 順序、算術、數字繞回去的那一瞬間 |

編輯器告訴你「有沒有成」，終端機告訴你「怎麼成的」。

這條規則是被一個做錯的建議逼出來的。0.5.0 做完播放鍵之後，
下一個提案是 **inlay hints**——在每一行行尾用幽靈文字顯示 register 的值，
你一邊打字一邊看著它繞回去。聽起來很棒，而且「別的語言插件做不出來」。

它是錯的，理由很精確：**那個逐步的過程正是「執行」唯一的內容。**
報告的價值就是 `2 - 3 on uint<2> = 3` 那一行。把它搬進環境層，
等於把按下播放鍵的唯一理由拿走。「這個東西很重要所以要一直顯示」的正確結論是
「這個東西很重要**所以它值得你按一下**」。

推論一下，這條規則把功能分成兩堆：

- **導航（大綱、跳定義）和文字工具（格式化器、snippets）安全。**
  它們不顯示結果，只幫你在文字裡移動。一分一毫都不侵蝕執行。
- **顯示結果的東西要很小心。** 每多一個，按播放鍵的理由就少一分。

它也輕微地指控已經做好的東西，而這裡不打算給自己豁免：
**CodeLens 站得住**，因為它是標題不是敘事——它說「有沒有成」。
**hover 的 register trail 就在線上**，它已經有一點「怎麼會」的味道。
哪天執行的理由變薄了，那是第一個該檢討的東西，不是最後一個。

唯一一種 inlay hints 是合理的：**掛在執行後面**，像除錯器那樣——
你跑過或單步之後才看到值，那是在放大動作而不是取代它。
